...
def handle_message(event):
    ...
    elif re.findall('沒有',event.message.text)[0] == '沒有':
        if re.findall('收',event.message.text)[0] == '收':
            if re.findall('保費',event.message.text)[0] == '保費':
                line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text='1. 您可以依繳費通知上記載之收費單位地址、電話逕與收費人員連絡。 2. 您可以撥打保戶服務專線0809-000-550與本公司連絡，來電時請留下保單號碼、電話及地址，以便通知收費人員與您連絡。'))

    #請問支票抬頭應如何開立？
    elif re.findall('支票',event.message.text)[0] == '支票':
        if re.findall('抬頭',event.message.text)[0] == '抬頭':
            if re.findall('開立|填寫',event.message.text)[0] == '開立|填寫':
                line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text='支票抬頭請依下列方式填寫： 1. 富邦人壽。 2. 富邦人壽保險（股）公司。 3. 富邦人壽保險股份有限公司。'))
                
    #我若移居國外，其收費情形應如何處理？
    elif re.findall('國外',event.message.text)[0] == '國外':
        if re.findall('收費',event.message.text)[0] == '收費':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #我遺失送金單時，該怎麼辦？
    elif re.findall('遺失|不見|補發',event.message.text)[0] == '遺失|不見|補發':
        if re.findall('送金單',event.message.text)[0] == '送金單':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #我的保單服務員是誰？
    elif re.findall('保單',event.message.text)[0] == '保單':
        if re.findall('服務員',event.message.text)[0] == '服務員':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('要保人',event.message.text)[0] == '要保人':
        if re.findall('變更',event.message.text)[0] == '變更':
            line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text='您詢問的險種有分成「傳統型」及「投資型」，請加入險種類型再詢問一次。'))
            
    elif re.findall('傳統型',event.message.text)[0] == '傳統型':
        if re.findall('要保人',event.message.text)[0] == '要保人':
            if re.findall('變更',event.message.text)[0] == '變更':
                line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text='應檢附之文件：(1) 契約變更申請書。 (2) 公司登記證明文件或商業登記證明文件。 (3) 證明文件（足以辨識要、被保人關係及是否具“保險利益”）。')) 
            
    elif re.findall('投資型',event.message.text)[0] == '投資型':
        if re.findall('要保人',event.message.text)[0] == '要保人':
            if re.findall('變更',event.message.text)[0] == '變更':
                line_bot_api.reply_message(
                event.reply_token,
    ...
    #要、被保人非同一人，要保人不幸死亡，應如何變更要保人？
        ...
        if re.findall('死亡',event.message.text)[0] == '死亡':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('錯誤',event.message.text)[0] == '錯誤':
        if re.findall('姓名|生日|姓別|身分證|資料',event.message.text)[0] == '姓名|生日|姓別|身分證|資料':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('更改|變更|更|改',event.message.text)[0] == '更改|變更|更|改':
        if re.findall('姓名|名',event.message.text)[0] == '姓名|名':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #保險費自動墊繳意願應如何變更？
    elif re.findall('保費|保險費',event.message.text)[0] == '保費|保險費':
        if re.findall('自動墊繳意願',event.message.text)[0] == '自動墊繳意願':
            line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text='1. 隨時均可提出申請，但若保單已進入自動墊繳，現欲變更為停止保險費自動墊繳，則墊繳意願變更之生效日為次一墊繳日。 2. 應備妥「契約變更申請書」，由要保人提出申請。'))

    #被保險人職業內容變動該如何辦理？
    elif re.findall('職業|工作',event.message.text)[0] == '職業|工作':
        if re.findall('更改|變更|變動|換',event.message.text)[0] == '更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #如何辦理地址之變更？
    elif re.findall('地址|住址',event.message.text)[0] == '地址|住址':
        if re.findall('更改|變更|變動|換',event.message.text)[0] == '更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #受益人可否變更？應如何申請？
    elif re.findall('受益人',event.message.text)[0] == '受益人':
        if re.findall('更改|變更|變動|換',event.message.text)[0] == '更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('減額繳清保險',event.message.text)[0] == '減額繳清保險':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    elif re.findall('展期定期保險',event.message.text)[0] == '展期定期保險':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    #辦理繳別變更，應備什麼文件及注意事項？    
    elif re.findall('繳別',event.message.text)[0] == '繳別':
        if re.findall('更改|變更|變動|換',event.message.text)[0] == '更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #何謂繳費方式變更？
    elif re.findall('繳費|繳費方式',event.message.text)[0] == '繳費|繳費方式':
        if re.findall('更改|變更|變動|換',event.message.text)[0] == '更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #保單遺失如何申請補發？
    elif re.findall('保單|保險單',event.message.text)[0] == '保單|保險單':
        if re.findall('遺失|不見|補發',event.message.text)[0] == '遺失|不見|補發':
            line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text='申請補發保單，應備文件如下： 1. 工本費。 2. 契約變更申請書。'))
        
    #主契約保險金額之縮小應如何提出申請？
    elif re.findall('保險|保費',event.message.text)[0] == '保險|保費':
        if re.findall('金額',event.message.text)[0] == '金額':
            if re.findall('縮小',event.message.text)[0] == '縮小':
                line_bot_api.reply_message(
                event.reply_token,
    ...
    elif re.findall('無|沒有',event.message.text)[0] == '無|沒有':
        if re.findall('附約',event.message.text)[0] == '附約':
            if re.findall('中途',event.message.text)[0] == '中途':
                line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text='視主約險種附加附約規定。（部分主約限定不得附加附約）'))
                    
    #保戶若申請附約取消或縮小保額時，可否退費？
    elif re.findall('附約',event.message.text)[0] == '附約':
        if re.findall('取消|中止|停止|縮小',event.message.text)[0] == '取消|中止|停止|縮小':
            if re.findall('退費',event.message.text)[0] == '退費':
                line_bot_api.reply_message(
                event.reply_token,
    ...
    #契約轉換應備哪些文件？
    #主契約可轉換之作業與險種規定為何？
    elif re.findall('契約',event.message.text)[0] == '契約':
        if re.findall('轉換|更改|變更|變動|換',event.message.text)[0] == '轉換|更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #什麼情形不得申請轉換契約？
    elif re.findall('不能|不得|不行|不可以|失敗',event.message.text)[0] == '不能|不得|不行|不可以|失敗':
        if re.findall('轉換|更改|變更|變動|換',event.message.text)[0] == '轉換|更改|變更|變動|換':
            if re.findall('契約',event.message.text)[0] == '契約':
                line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text='1. 停效件須先辦理復效。 2.已變更為展期或減額繳清保險者。 3. 繳費期滿件或躉繳件。 4. 已享有豁免保費者。'))
                    
    #轉換後契約之投保始期與保險年齡如何計算？
    elif re.findall('轉換|更改|變更|變動|換',event.message.text)[0] == '轉換|更改|變更|變動|換':
        if re.findall('契約',event.message.text)[0] == '契約':
            if re.findall('年齡|時間|始期',event.message.text)[0] == '年齡|時間|始期':
                line_bot_api.reply_message(
                event.reply_token,
    ...
    #申請保險年期變更，應備哪些文件？
    #年期變更之規定為何？
    elif re.findall('年期',event.message.text)[0] == '年期':
        if re.findall('轉換|更改|變更|變動|換',event.message.text)[0] == '轉換|更改|變更|變動|換':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #契約「停效」後，該如何申請復效？
        ...
        if re.findall('復效',event.message.text)[0] == '復效':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #如何辦理國外理賠件？
        ...
        if re.findall('理賠',event.message.text)[0] == '理賠':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('診斷書',event.message.text)[0] == '診斷書':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    elif re.findall('未成年',event.message.text)[0] == '未成年':
        if re.findall('理賠|申請書',event.message.text)[0] == '理賠|申請書':
            if re.findall('填寫|申請',event.message.text)[0] == '填寫|申請':
                line_bot_api.reply_message(
                event.reply_token,
    ...
    elif re.findall('死亡',event.message.text)[0] == '死亡':
        if re.findall('受益人',event.message.text)[0] == '受益人':
            if re.findall('未成年',event.message.text)[0] == '未成年':
                if re.findall('理賠',event.message.text)[0] == '理賠':
                    line_bot_api.reply_message(
                    event.reply_token,
    ...
    #住院日數如何計算？
    elif re.findall('住院',event.message.text)[0] == '住院':
        if re.findall('日數|天數',event.message.text)[0] == '日數|天數':
            line_bot_api.reply_message(
            event.reply_token,
            TextSendMessage(text='住院日數之計算採計頭計尾，例如1/1入院至1/3出院，住院日數為3天。'))
    
    #當日出院後於當天又再度入院，住院天數算幾天？
    elif re.findall('出院',event.message.text)[0] == '出院':
        if re.findall('又|再',event.message.text)[0] == '又|再':
            if re.findall('住院',event.message.text)[0] == '住院':
                if re.findall('日數|天數',event.message.text)[0] == '日數|天數':
                    line_bot_api.reply_message(
                    event.reply_token,
    ...
    #包皮過長去醫院切除並住院，可否理賠？
    elif re.findall('包皮',event.message.text)[0] == '包皮':
        line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text='若有合併發炎情形則可給付。')) 
                
    #收據遺失是否可以副本或補發收據代替申請？
    elif re.findall('收據',event.message.text)[0] == '收據':
        if re.findall('遺失|不見|補發',event.message.text)[0] == '遺失|不見|補發':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #中醫診所住院治療，其理賠給付準則如何？
    elif re.findall('中醫',event.message.text)[0] == '中醫':
        if re.findall('住院',event.message.text)[0] == '住院':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('蝦子',event.message.text)[0] == '蝦子':
        if re.findall('刺傷',event.message.text)[0] == '刺傷':
            if re.findall('食道',event.message.text)[0] == '食道':    
                if re.findall('意外',event.message.text)[0] == '意外':
                    line_bot_api.reply_message(
                    event.reply_token,
    ...
    elif re.findall('電|電擊',event.message.text)[0] == '電|電擊':
        if re.findall('意外',event.message.text)[0] == '意外':  
            line_bot_api.reply_message(
            event.reply_token,
    ...
    #事故人被人砍傷，是否可給付？
    elif re.findall('砍|砍傷',event.message.text)[0] == '砍|砍傷':
        line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text='若可排除除外責任則可給付。'))                            
                        
    #中暑是否可申請意外傷害醫療保險金？
    elif re.findall('中暑',event.message.text)[0] == '中暑':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    elif re.findall('椎間盤突出',event.message.text)[0] == '椎間盤突出':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    #良性腫瘤是否可申請癌症保險金？
    elif re.findall('良性腫瘤',event.message.text)[0] == '良性腫瘤':
        line_bot_api.reply_message(
        event.reply_token,
        TextSendMessage(text='良性腫瘤非癌症不符合癌症保險金給付條件。'))             
                   
    #若保戶為醫師，申請癌症保險金有無特別限制？        
    elif re.findall('要保人|被保人|受益人|保戶',event.message.text)[0] == '要保人|被保人|受益人|保戶':
        if re.findall('醫生|醫師',event.message.text)[0] == '醫生|醫師':
            line_bot_api.reply_message(
            event.reply_token,
    ...
    elif re.findall('酒',event.message.text)[0] == '酒':
        if re.findall('駕',event.message.text)[0] == '駕':
            if re.findall('理賠',event.message.text)[0] == '理賠':    
                line_bot_api.reply_message(
                event.reply_token,
    ...
    elif re.findall('心肌梗塞',event.message.text)[0] == '心肌梗塞':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    elif re.findall('失蹤',event.message.text)[0] == '失蹤':   
        line_bot_api.reply_message(
        event.reply_token,
    ...
    elif re.findall('植物人',event.message.text)[0] == '植物人':
        line_bot_api.reply_message(
        event.reply_token,
    ...
    #何謂法定繼承人？        
    elif re.findall('法定繼承人',event.message.text)[0] == '法定繼承人':
        line_bot_api.reply_message(
        event.reply_token,
