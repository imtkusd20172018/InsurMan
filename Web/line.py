...
def add_numbers():
    ...
    try:
        ...
        elif(re.findall('要保人', query)!=None):
            if (re.findall('變更', query)!=None):
                reply = '您詢問的險種有分成「傳統型」及「投資型」，請加入險種類型再詢問一次。'
        elif(re.findall('投資型', query)!=None):
            if (re.findall('要保人', query)!=None):
                if (re.findall('變更', query)!=None):
        ...
        elif(re.findall('傳統型', query)!=None):
            if (re.findall('要保人', query)!=None):
                if (re.findall('變更', query)!=None):
        ...
        #保險單上要保人或被保險人姓名、出生日期、性別或身分證號碼錯誤時，應如何處理？
            elif(re.findall('錯誤', query)!=None):
                if (re.findall('姓名|生日|姓別|身分證|資料', query)!=None):
        ...
        #要保人（或被保險人）更改名時，應如何處理？
            elif(re.findall('更改|變更|更|改', query)!=None):
            if (re.findall('姓名|名', query)!=None):
